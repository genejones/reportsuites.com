...
function createSummarySheet(wb, report_suites){
	var timeGeneratedAt = new Date();
	var timeGeneratedAtString = timeGeneratedAt.toLocaleString('en-US', {timeZoneName:'short'});
	var array = [ [{value:"Adobe Analytics Multi-Report Suite Configuration Report", "style":{font:{sz:18, bold:'true'}}}], [['']], ["Generated by Gene Jones's reportsuites.com"], ["Generated at " + timeGeneratedAtString] ];
	array.push(['']);
	array.push(["Report-suites summarized"]);
...
var createOverviewOfSlot = function(report_suites, inputNVP, slotName, workbook){
	...
	for (let i=0; i<report_suites.length; i++){
		allKeys = uniq(keys(inputNVP[report_suites[i].rsid]), keys);
	}
...
function exportSiteCatToExcel(report_suites, evarArray, propArray, eventArray, fileName, callback) {
	...
	createSummarySheet(workbook, report_suites);
